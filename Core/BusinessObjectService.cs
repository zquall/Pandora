...
namespace Core
{
    ...
    {
        public string GetBusinessObjectXmlSchema(string sessionId)
        {
            ...
            var objectType = "oCreditNotes";
            var soapCommand = @"<?xml version=""1.0"" encoding=""UTF-16""?>" +
                                 @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + 
                                    "<env:Header><SessionID>" + sessionId + "</SessionID></env:Header>" +
                                @"<env:Body><dis:GetBusinessObjectXmlSchema xmlns:dis=""http://www.sap.com/SBO/DIS"">" +
                                    "<Object>" + objectType + "</Object>" +
                                "</dis:GetBusinessObjectXmlSchema></env:Body></env:Envelope>";
